...
class _FlutterSliderState extends State<FlutterSlider>
  ...
  void _setParameters() {
    ...
    _tooltipData.textStyle ??= TextStyle(fontSize: 12, color: Colors.black);
...
//    }
    ...
    _leftHandlerScaleAnimationController = AnimationController(duration: widget.handlerAnimation.duration,value: this);
    _rightHandlerScaleAnimationController = AnimationController(
        duration: widget.handlerAnimation.duration, value: this);
    ...
    _leftTooltipAnimationController =
        AnimationController(duration: Duration(milliseconds: 200), value: this);
...
//                  overflow: TextOverflow.visible,
  ...
  void didUpdateWidget(FlutterSlider oldWidget) {
    ...
    if ((lowerValueValidation && upperValueValidation) &&
        (lowerValueNotEquality ||
            upperValueNotEquality ||
            _originalMin != _widgetMin ||
            _originalMax != _widgetMax ||
            _originalLeftHandler != widget.handler.toString() ||
            _originalRightHandler != widget.rightHandler.toString() ||
...
//          reArrangePositions = true;
    ...
    if (_constraintMaxHeight != null && _constraintMaxWidth != null) {
      _drawHatchMark();
    }
      ...
      builder: (context, orientation) {
        ...
        return LayoutBuilder(
            builder: (BuildContext context, BoxConstraints constraints) {
              ...
              return Container(
                padding:
                EdgeInsets.only(top: _handlersPadding),
                key: containerKey,
...
//        __axisPosTmp - tS <= __rightHandlerPosition + 1 &&
        ((__axisPosTmp + _handlersPadding >= _handlersPadding - 1) ||
            forcePosStop) /* - _leftPadding*/
    ) {
      ...
      if (widget.jump == true) {
        if (!forcePosStop) {
          ...
          if (__axisPosTmp - tS >= __rightHandlerPosition) {
            ...
            __leftHandlerPosition = -(tS - _handlersWidth / 2);
          }
...
//        __leftHandlerPosition = __dAxis - __axisDragTmp;
  ...
  void _rightHandlerMove(PointerEvent pointer,
    ...
    bool forcePosStop = false;
    if (((__axisPosTmp >= __containerSizeWithoutPadding + _handlersPadding) ||
      ...
      if (widget.jump == true) {
        if (!forcePosStop) {
          ...
          if (__axisPosTmp - tS <= __leftHandlerPosition) {
            ...
            if (widget.axis == Axis.horizontal) {
              __rightHandlerPosition = _containerWidthWithoutPadding -
                  _touchSize +
                  (MediaQuery.of(context).size.width / _constraintMaxWidth) /
                      (MediaQuery.of(context).size.width / _constraintMaxWidth);
            } else {
              __rightHandlerPosition = _containerHeightWithoutPadding -
                  _touchSize +
...
//    }
  ...
  Positioned _inactiveTrack() {
    ...
    Color trackBarColor = boxDecoration.color ?? Colors.black12;
    if (widget.disabled)
  ...
  Positioned _activeTrack() {
    ...
    Color trackBarColor = boxDecoration.color ?? Colors.black26;
    if (widget.disabled)
      trackBarColor = widget.barcolor;
    ...
    if (widget.axis == Axis.horizontal)
    {
      ...
      if (widget.rtl == true && widget.rangeSlider == false) {
        ...
        width =
            _containerWidthWithoutPadding - _rightHandlerXPosition;
      }
    }
    else
      {
...
class _MakeHandler extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
   ...
   /* if (handlerIndex == 2) {
      handler.child ??= Icon(
          (axis == Axis.horizontal) ?
          Icons.crop_16_9: Icons.expand_more,// Icon Add Sign
          color: Colors.black45);
    } else {
      IconData hIcon =
